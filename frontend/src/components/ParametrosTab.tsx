...
interface DolarData {
  oficial: { compra: number; venta: number; promedio: number };
  blue: { compra: number; venta: number; promedio: number };
}
...
const ParametrosTab: React.FC<ParametrosTabProps> = ({
  ...
  // Estados para el selector de dólar
  const [dolarData, setDolarData] = useState<DolarData | null>(null);
  const [loadingDolar, setLoadingDolar] = useState(true);
  const [tipoDolar, setTipoDolar] = useState<'oficial' | 'blue'>('blue');
  const [tipoValor, setTipoValor] = useState<'compra' | 'venta' | 'promedio'>('venta');
  
  // Calcular el valor del dólar seleccionado
  const dolarSeleccionado = dolarData 
    ? dolarData[tipoDolar][tipoValor]
    : null;
      ...
      try {
        ...
        setDolarData({
          oficial: {
            ...
            venta: oficial.venta,
            promedio: (oficial.compra + oficial.venta) / 2
          },
          blue: {
            compra: blue.compra,
            venta: blue.venta,
            promedio: (blue.compra + blue.venta) / 2
          }
        });
      } catch (error) {
        console.error('Error fetching dólar data:', error);
      } finally {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateConfigMutation.mutate({
      ...
      tipo_cambio_usd_ars: dolarSeleccionado || Number(formData.get('tipo_cambio')) || 1335,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Configura cómo se calcula el costo por hora de tu consultorio
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass rounded-2xl p-6 shadow-soft border border-white/20">
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
            <TrendingUp className="h-5 w-5 text-dental-500" />
            Modo de Cálculo del Costo por Hora
          </h3>
